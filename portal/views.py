...
def dashboard_redirect(request):
    if request.user.is_superuser or request.user.role == User.IS_ADMIN:
...
def admin_dashboard(request):
    if not (request.user.is_superuser or request.user.role == User.IS_ADMIN):
        return redirect('home')
    
    stats = {
        'total_users': User.objects.exclude(is_superuser=True).exclude(role='admin').count(),
    ...
    recent_users = User.objects.exclude(is_superuser=True).exclude(role='admin').order_by('-date_joined')[:5]
...
def admin_jobs(request):
    if not (request.user.is_superuser or request.user.role == User.IS_ADMIN): return redirect('home')
...
def admin_users(request):
    if not (request.user.is_superuser or request.user.role == User.IS_ADMIN): return redirect('home')
    users = User.objects.exclude(is_superuser=True).exclude(role='admin').order_by('-date_joined')
